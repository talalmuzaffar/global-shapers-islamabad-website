...
// ==================== DYNAMIC EVENTS (HOMEPAGE) ====================

document.addEventListener('DOMContentLoaded', async function () {
    const eventsGrid = document.getElementById('dynamic-events');
    if (!eventsGrid) return;

    const events = await loadEvents();
    if (!events.length) {
        eventsGrid.innerHTML = '<p style="text-align:center;color:var(--text-gray);">No events to display.</p>';
        return;
    }
    ...
    eventsGrid.innerHTML = events.map(ev => `
        <div class="event-card">
            <div class="event-image">
                ${ev.imageUrl
                    ? '<img src="' + ev.imageUrl + '" alt="' + escapeHtmlAttr(ev.title) + '" loading="lazy">'
                    : '<div class="project-placeholder"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="48" height="48"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg></div>'
                }
            </div>
            <div class="event-card-content">
                ${ev.date ? '<span class="event-date">' + escapeHtmlContent(ev.date) + (ev.location ? ' &bull; ' + escapeHtmlContent(ev.location) : '') + '</span>' : ''}
                <h3 class="event-title">${escapeHtmlContent(ev.title)}</h3>
                <p class="event-description">${escapeHtmlContent(ev.description)}</p>
                ${ev.upcoming ? '<span class="event-upcoming-badge">Upcoming</span>' : ''}
            </div>
        </div>
    `).join('');
});
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    const instagramSvg = '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>';
    const facebookSvg = '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>';
    ...
    teamContainer.innerHTML = activeMembers.map(m => {
        const socialLinks = [];
        if (m.socials?.linkedin) socialLinks.push('<a href="' + m.socials.linkedin + '" target="_blank" rel="noopener" class="member-social-link linkedin" aria-label="LinkedIn">' + linkedinSvg + '</a>');
        if (m.socials?.instagram) socialLinks.push('<a href="' + m.socials.instagram + '" target="_blank" rel="noopener" class="member-social-link instagram" aria-label="Instagram">' + instagramSvg + '</a>');
        if (m.socials?.facebook) socialLinks.push('<a href="' + m.socials.facebook + '" target="_blank" rel="noopener" class="member-social-link facebook" aria-label="Facebook">' + facebookSvg + '</a>');
        ...
        <div class="member-card">
            <div class="member-card-avatar">
                ${m.photoUrl
                    ...
                    : '<div class="member-avatar-initials">' + getInitials(m.name) + '</div>'
                }
            </div>
            <h3 class="member-card-name">${escapeHtmlContent(m.name)}</h3>
            ${m.role ? '<span class="member-card-role">' + escapeHtmlContent(m.role) + '</span>' : ''}
            ${socialLinks.length > 0 ? '<div class="member-card-socials">' + socialLinks.join('') + '</div>' : ''}
        </div>`;
    }).join('');
});
